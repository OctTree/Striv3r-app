class Api::V1::ActivityPlansController < ApplicationController
  ...
  before_action :set_activity_plan, only: %i[update show]
  ...
  def update
    if @activity_plan.update(activity_plans_params)
      ActivityLog.create(user_id: current_user.id, minutes: @activity_plan.time, date_at: DateTime.current, name: @activity_plan.activity_name) if @activity_plan.completed
      @activity_plan.update(completed: false) unless @activity_plan.frequency_finished == @activity_plan.frequency
  ...
  def activity_plans_params
    params.require(:activity_plan).permit( :time_spent, :remaining_time, :frequency_finished, :completed)
  end
end